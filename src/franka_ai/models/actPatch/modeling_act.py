...
class ACTPolicy(PreTrainedPolicy):
    ...
    @torch.no_grad
    def predict_actions(self, queue) -> Tensor:
        """Select a single action given environment observations.
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.use_vae and "action" in batch:
            ...
            if self.config.robot_state_feature:
                ...
                #print(robot_state_embed.shape, batch["observation.state"].shape)
                ##TODO:BIG MODIF
        ...
        if self.config.robot_state_feature:
            #TODO:BIG MODIF
            #print("obs", batch["observation.state"].shape)
...
class ACTRgbEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.in_features = backbone_model.heads.head.in_features
